/*
 * $Id: drawtext.c 2951 2021-03-02 14:19:33Z df7be $
 *
...
HB_FUNC( HWG_DRAWTEXT )
{
   ...
   if( hb_parclen(2) > 0 )
   {
      ...
      hwg_setcolor(hDC->cr, (hDC->fcolor != -1) ? hDC->fcolor : 0);
      cairo_move_to(hDC->cr, static_cast<gdouble>(hb_parni(3)), static_cast<gdouble>(hb_parni(4)));
      pango_cairo_show_layout(hDC->cr, hDC->layout);
      g_free(cText);
   }
}
...
HB_FUNC( HWG_GETTEXTMETRIC )
{
   ...
   }
   {
      ...
      metrics = pango_context_get_metrics(context, hDC->hFont, nullptr);
      ...
      temp = _itemPutNL(nullptr, height);
      _itemArrayPut(aMetr, 1, temp);
      _itemRelease(temp);

      temp = _itemPutNL(nullptr, width);
...
{
   GtkWidget * hCtrl = static_cast<GtkWidget*>(HB_PARHANDLE(1));
   PHWGUI_FONT pFont = static_cast<PHWGUI_FONT>(HB_PARHANDLE(3));
   char szData[256];
   ...
   if( pName && strncmp(pName, "Gtk", 3) != 0 )
   {
      ...
      sprintf(szData, "#%s { font-family: %s; font-size: %dpx; font-weight: %s;  font-style: %s; }",
         pName, szFamily, iHeight/PANGO_SCALE, (iBold) ? "bold" : "normal", (iIta) ? "italic" : "normal");
