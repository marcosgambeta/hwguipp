...
void long2rgb(long int nColor, double *pr, double *pg, double *pb)
{
  ...
  *pr = (static_cast<double>(r)) / 255.;
  *pg = (static_cast<double>(g)) / 255.;
  *pb = (static_cast<double>(b)) / 255.;
}
...
static void draw_page(cairo_t *cr, const char *cpage)
{
  ...
  while (*ptr) {
    if (!strncmp(ptr, "txt", 3)) {
      ...
      cairo_move_to(cr, static_cast<gdouble>(x1), static_cast<gdouble>(y2));
      ...
      cairo_rectangle(cr, static_cast<gdouble>(x1), static_cast<gdouble>(y1), static_cast<gdouble>(x2 - x1 + 1),
                      static_cast<gdouble>(y2 - y1 + 1));
      iPathExist = 1;
...
static void print_run(GtkWidget *widget)
{
  if (!g_object_get_data(static_cast<GObject *>(widget), "flag")) {
    ...
    print->count = 0;
    g_timeout_add(1000, G_CALLBACK(print_time), static_cast<gpointer>(widget));
...
static void print_init(GtkPrintOperation *operation, PHWGUI_PRINT print)
{
  ...
  g_signal_connect_swapped(G_OBJECT(prnwindow), "destroy", G_CALLBACK(gtk_main_quit), nullptr);

  g_object_set_data(static_cast<GObject *>(prnwindow), "oper", static_cast<gpointer>(operation));
  g_object_set_data(static_cast<GObject *>(prnwindow), "print", static_cast<gpointer>(print));

  g_signal_connect(prnwindow, "focus_in_event", G_CALLBACK(print_run), nullptr);
...
HB_FUNC(HWG_GP_PRINT)
{
  ...
  if (!iOper) {
    ...
    if (iOper == 2) {
      surface = cairo_ps_surface_create(print->cName, gtk_page_setup_get_page_width(print->page_setup, GTK_UNIT_POINTS),
                                        gtk_page_setup_get_page_height(print->page_setup, GTK_UNIT_POINTS));
    } else {
      surface =
          cairo_svg_surface_create(print->cName, gtk_page_setup_get_page_width(print->page_setup, GTK_UNIT_POINTS),
                                   gtk_page_setup_get_page_height(print->page_setup, GTK_UNIT_POINTS));
    }
...
HB_FUNC(HWG_GP_GETDEVICEAREA)
{
  ...
  auto temp =
      hb_itemPutNL(nullptr, static_cast<HB_LONG>(gtk_page_setup_get_page_width(print->page_setup, GTK_UNIT_POINTS)));
  hb_itemArrayPut(aMetr, 1, temp);

  hb_itemPutNL(temp, static_cast<HB_LONG>(gtk_page_setup_get_page_height(print->page_setup, GTK_UNIT_POINTS)));
  hb_itemArrayPut(aMetr, 2, temp);

  hb_itemPutNL(temp, static_cast<HB_LONG>(gtk_page_setup_get_page_width(print->page_setup, GTK_UNIT_MM)));
  hb_itemArrayPut(aMetr, 3, temp);

  hb_itemPutNL(temp, static_cast<HB_LONG>(gtk_page_setup_get_page_height(print->page_setup, GTK_UNIT_MM)));
...
HB_FUNC(HWG_GP_GETTEXTSIZE)
{
  ...
  hb_retnl(static_cast<HB_LONG>(exten.width));
  g_free(cText);
}
