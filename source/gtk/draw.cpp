...
void hwg_setcolor(cairo_t *cr, long int nColor)
{
  ...
  cairo_set_source_rgb(cr, (static_cast<double>(r)) / 255., (static_cast<double>(g)) / 255.,
...
HB_FUNC(HWG_RECTANGLE)
{
  ...
  auto x1 = hb_parni(2);
  auto y1 = hb_parni(3);
  cairo_rectangle(hDC->cr, static_cast<gdouble>(x1), static_cast<gdouble>(y1),
                  static_cast<gdouble>(hb_parni(4) - x1 + 1), static_cast<gdouble>(hb_parni(5) - y1 + 1));
  cairo_stroke(hDC->cr);
}
...
HB_FUNC(HWG_ELLIPSE)
{
  ...
  auto x2 = hb_parni(4);
  auto y2 = hb_parni(5);
  cairo_arc(hDC->cr, static_cast<double>(x1) + (x2 - x1) / 2, static_cast<double>(y1) + (y2 - y1) / 2,
            static_cast<double>(x2 - x1) / 2, 0, 6.28);
  cairo_stroke(hDC->cr);
}
...
HB_FUNC(HWG_DRAWBUTTON)
{
  ...
  if (iType == 0)
  {
    ...
    cairo_rectangle(hDC->cr, static_cast<gdouble>(left), static_cast<gdouble>(top),
                    static_cast<gdouble>(right - left + 1), static_cast<gdouble>(bottom - top + 1));
    cairo_fill(hDC->cr);
  }
  else
  {
...
{
  hwg_gtk_drawedge(static_cast<PHWGUI_HDC>(hb_parptr(1)), hb_parni(2), hb_parni(3), hb_parni(4), hb_parni(5),
                   hb_parni(6));
}
...
HB_FUNC(HWG_WINDOW2BITMAP)
{
  ...
  pixbuf = gdk_pixbuf_get_from_drawable(pixbuf, gtk_widget_get_window(hCtrl), nullptr, hb_parni(2), hb_parni(3), 0, 0,
...
HB_FUNC(HWG_CREATECOMPATIBLEDC)
{
  ...
  hDCdest->surface = cairo_surface_create_similar(cairo_get_target(hDCsource->cr), CAIRO_CONTENT_COLOR_ALPHA,
  ...
  else
  {
    ...
    for (gint i = 0; i < colors_num; i++)
    {
      ...
      stop = (i < user_stops_num) ? hb_arrayGetND(pArrStop, i + 1)
                                  : 1. / static_cast<gdouble>(colors_num - 1) * static_cast<gdouble>(i);
