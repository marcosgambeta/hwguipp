...
#define ListView_SortItemsEx(hwndLV, _pfnCompare, _lPrm)                                                               \
  (BOOL) SNDMSG((hwndLV), LVM_SORTITEMSEX, static_cast<WPARAM>(static_cast<LPARAM>(_lPrm)),                            \
                static_cast<LPARAM>((PFNLVCOMPARE)(_pfnCompare)))
#endif
...
HB_FUNC(HWG_LISTVIEW_CREATE)
{
  ...
  auto handle = CreateWindowEx(
      WS_EX_CLIENTEDGE, WC_LISTVIEW, nullptr, style, hwg_par_int(3), hwg_par_int(4), hwg_par_int(5), hwg_par_int(6),
...
HB_FUNC(HWG_LISTVIEW_SETVIEW)
{
  ...
  if ((dwStyle & LVS_TYPEMASK) != dwView)
  {
    ...
    //  RedrawWindow(hwg_par_HWND(1), nullptr, nullptr, RDW_ERASE | RDW_INVALIDATE | RDW_ALLCHILDREN | RDW_ERASENOW |
    //  RDW_UPDATENOW);
  }
}
...
HB_FUNC(HWG_LISTVIEW_ADDCOLUMNEX)
{
  ...
  if (SendMessage(static_cast<HWND>(hwndListView), LVM_INSERTCOLUMN, static_cast<WPARAM>(static_cast<int>(lCol)),
                  reinterpret_cast<LPARAM>(&lvcolumn)) == -1)
  {
    iResult = 0;
  }
  else
