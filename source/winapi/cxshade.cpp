...
long cxdib_Draw(PCXDIB pdib, HDC pDC, long xoffset, long yoffset)
{
  if (pdib->hDib && pDC) {
    ...
    SetDIBitsToDevice(pDC, xoffset, yoffset, pdib->m_bi.biWidth, pdib->m_bi.biHeight, 0, 0, 0, pdib->m_bi.biHeight,
                      cxdib_GetBits(pdib), reinterpret_cast<BITMAPINFO *>(lpDIB), DIB_RGB_COLORS);
    return 1;
  }
  return 0;
}
...
long cxdib_Stretch(PCXDIB pdib, HDC pDC, long xoffset, long yoffset, long xsize, long ysize)
{
  if (pdib->hDib && pDC) {
    ...
    StretchDIBits(pDC, xoffset, yoffset, xsize, ysize, 0, 0, pdib->m_bi.biWidth, pdib->m_bi.biHeight,
                  cxdib_GetBits(pdib), reinterpret_cast<BITMAPINFO *>(lpDIB), DIB_RGB_COLORS, SRCCOPY);
    return 1;
  }
  return 0;
}
                      ...
                      COLORREF color, RECT *prect)
{
  ...
  for (i = 0; i < 129; i++) {
    ...
    cxdib_SetPaletteIndex(&(pshade->m_dNormal), static_cast<BYTE>(i), static_cast<BYTE>(r), static_cast<BYTE>(g),
                          static_cast<BYTE>(b));
    cxdib_SetPaletteIndex(&(pshade->m_dh), static_cast<BYTE>(i), static_cast<BYTE>(r), static_cast<BYTE>(g),
                          static_cast<BYTE>(b));
    cxdib_SetPaletteIndex(&(pshade->m_dv), static_cast<BYTE>(i), static_cast<BYTE>(r), static_cast<BYTE>(g),
                          static_cast<BYTE>(b));
  }
  for (i = 1; i < 129; i++) {
    ...
    cxdib_SetPaletteIndex(&(pshade->m_dNormal), static_cast<BYTE>(i + 127), static_cast<BYTE>(r), static_cast<BYTE>(g),
                          static_cast<BYTE>(b));
    cxdib_SetPaletteIndex(&(pshade->m_dh), static_cast<BYTE>(i + 127), static_cast<BYTE>(r), static_cast<BYTE>(g),
                          static_cast<BYTE>(b));
    cxdib_SetPaletteIndex(&(pshade->m_dv), static_cast<BYTE>(i + 127), static_cast<BYTE>(r), static_cast<BYTE>(g),
                          static_cast<BYTE>(b));
  }
...
// HWG_SHADE_SET(pshade, shadeID, palette, granularity, highlight, coloring, color, nLeft, nTop, nRight, nBottom) -->
// NIL
