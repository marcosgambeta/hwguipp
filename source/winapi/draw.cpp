...
HB_FUNC(HWG_PIE)
{
  int res = Pie(hwg_par_HDC(1), hb_parni(2), hb_parni(3), hb_parni(4), hb_parni(5), hb_parni(6), hb_parni(7),
...
HB_FUNC(HWG_FILLRECT)
{
  ...
  FillRect(hwg_par_HDC(1), &rc,
           HB_ISPOINTER(6) ? hwg_par_HBRUSH(6)
...
HB_FUNC(HWG_DRAWBUTTON)
{
  ...
  else
  {
    ...
    rc.top++;
    FillRect(hDC, &rc,
             reinterpret_cast<HBRUSH>(((iType & 2)   ? COLOR_3DHILIGHT
...
HB_FUNC(HWG_LOADIMAGE)
{
  ...
  hb_retptr(LoadImage(
      HB_ISNIL(1) ? GetModuleHandle(nullptr)
                  : reinterpret_cast<HINSTANCE>(hb_parnl(1)), // handle of the instance that contains the image
...
HB_FUNC(HWG_OPENBITMAP)
{
  ...
  HANDLE hfbm = CreateFile(HB_PARSTR(1, &hString, nullptr), GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING,
...
HB_FUNC(HWG_SAVEBITMAP)
{
  ...
  *lpbi = bi;
  ...
  GetDIBits(hDC, hBitmap, 0, static_cast<UINT>(Bitmap0.bmHeight),
            reinterpret_cast<LPSTR>(lpbi) + sizeof(BITMAPINFOHEADER) + dwPaletteSize,
  ...
  bmfHdr.bfOffBits =
      static_cast<DWORD>(sizeof(BITMAPFILEHEADER)) + static_cast<DWORD>(sizeof(BITMAPINFOHEADER)) + dwPaletteSize;
...
HB_FUNC(HWG_BITBLT)
{
  hb_retl(BitBlt(hwg_par_HDC(1), hb_parni(2), hb_parni(3), hb_parni(4), hb_parni(5), hwg_par_HDC(6), hb_parni(7),
