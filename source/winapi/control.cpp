...
HB_FUNC(HWG_MOVEWINDOW)
{
  ...
  MoveWindow(hwg_par_HWND(1), (HB_ISNIL(2)) ? rc.left : hb_parni(2), (HB_ISNIL(3)) ? rc.top : hb_parni(3),
             (HB_ISNIL(4)) ? rc.right - rc.left : hb_parni(4), (HB_ISNIL(5)) ? rc.bottom - rc.top : hb_parni(5),
...
{
  auto hWndPanel =
      CreateWindowEx(0, TEXT("OWNBTN"), nullptr, WS_CHILD | WS_VISIBLE | SS_GRAYRECT | SS_OWNERDRAW, hwg_par_int(3),
                     hwg_par_int(4), hwg_par_int(5), hwg_par_int(6), hwg_par_HWND(1),
...
HB_FUNC(HWG_CREATEBROWSE)
{
  ...
  void *hStr;
  auto hWndBrw = CreateWindowEx(
      (dwStyle & WS_BORDER) ? WS_EX_CLIENTEDGE : 0, TEXT("BROWSE"), HB_PARSTR(8, &hStr, nullptr),
      WS_CHILD | WS_VISIBLE | dwStyle, hwg_par_int(4), hwg_par_int(5), hwg_par_int(6), hwg_par_int(7), hwg_par_HWND(1),
      reinterpret_cast<HMENU>(static_cast<UINT_PTR>(hb_parni(2))), GetModuleHandle(nullptr), nullptr);
  hb_strfree(hStr);
  hb_retptr(hWndBrw);
}
...
HB_FUNC(HWG_CREATESTATUSWINDOW)
{
  ...
  auto hwndStatus = CreateWindowEx(
      0, STATUSCLASSNAME, nullptr, SBARS_SIZEGRIP | WS_CHILD | WS_VISIBLE | WS_OVERLAPPED | WS_CLIPSIBLINGS, 0, 0, 0, 0,
...
{
  wpOrigListProc = reinterpret_cast<WNDPROC>(
      SetWindowLongPtr(hwg_par_HWND(1), GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(ListSubclassProc)));
}

HB_FUNC(HWG_INITTRACKPROC)
{
  wpOrigTrackProc = reinterpret_cast<WNDPROC>(
      SetWindowLongPtr(hwg_par_HWND(1), GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(TrackSubclassProc)));
}
...
HB_FUNC(HWG_CREATEPAGER)
{
  ...
  auto hWndPanel =
      CreateWindowEx(0, WC_PAGESCROLLER, nullptr, WS_CHILD | WS_VISIBLE | bVert ? PGS_VERT : PGS_HORZ | hb_parnl(3),
...
HB_FUNC(HWG_CREATEREBAR)
{
  ...
  auto hWndCtrl = CreateWindowEx(
      ulExStyle, REBARCLASSNAME, nullptr,
...
{
  void *hStr;
  hb_retl(AddBar1(hwg_par_HWND(1), hwg_par_HWND(2), hwg_par_COLORREF(3), hwg_par_COLORREF(4),
                  HB_PARSTR(5, &hStr, nullptr), hb_parnl(6)));
  hb_strfree(hStr);
}
...
HB_FUNC(HWG_GETUTCTIMEDATE) // Format: W,YYYYMMDD-HH:MM:SS
{
  ...
  TCHAR cst[41] = {0};
  GetSystemTime(&st);
  sprintf((char *)cst, "%01d.%04d%02d%02d-%02d:%02d:%02d", st.wDayOfWeek, st.wYear, st.wMonth, st.wDay, st.wHour,
