...
HB_FUNC(HWG_CREATERICHEDIT)
{
  ...
  auto hCtrl = CreateWindowEx(
      ...
      TEXT("RichEdit20A"),
#endif
      nullptr, WS_CHILD | WS_VISIBLE | hb_parnl(3), hwg_par_int(4), hwg_par_int(5), hwg_par_int(6), hwg_par_int(7),
...
HB_FUNC(HWG_RE_FINDTEXT)
{
  ...
  auto lPos =
      static_cast<LONG>(SendMessage(hCtrl, EM_FINDTEXTEX, static_cast<WPARAM>(lFlag), reinterpret_cast<LPARAM>(&ft)));
  hb_strfree(hString);
  hb_retnl(lPos);
}
...
{
  wpOrigRichProc = reinterpret_cast<WNDPROC>(
      SetWindowLongPtr(hwg_par_HWND(1), GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(RichSubclassProc)));
}
