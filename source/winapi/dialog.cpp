...
HB_FUNC(HWG_DIALOGBOX)
{
  ...
  DialogBoxParam(hModule, lpResource, hwg_par_HWND(1), reinterpret_cast<DLGPROC>(s_ModalDlgProc),
...
HB_FUNC(HWG_CREATEDIALOG)
{
  ...
  hDlg = CreateDialogParam(hModule, lpResource, hwg_par_HWND(1), reinterpret_cast<DLGPROC>(s_DlgProc),
                           reinterpret_cast<LPARAM>(pObject));
  hb_strfree(hResource);
...
{
  return hb_itemCopyStrU16(pItem, HB_CDP_ENDIAN_NATIVE, reinterpret_cast<HB_WCHAR *>(lpWCStr), size) + 1;
}
...
static LPDLGTEMPLATE s_CreateDlgTemplate(PHB_ITEM pObj, int x1, int y1, int dwidth, int dheight, ULONG ulStyle)
{
  ...
  *p++ = LOWORD(ulStyle);
  *p++ = HIWORD(ulStyle);
  *p++ = static_cast<UINT>(ulControls); // NumberOfItems
  *p++ = x1;                            // x
  *p++ = y1;                            // y
  *p++ = dwidth;                        // cx
  *p++ = dheight;                       // cy
  ...
  *p = 0; // Number of bytes of extra data.
  ...
  return static_cast<LPDLGTEMPLATE>(hgbl);
}
...
{
  hb_retnl(reinterpret_cast<LONG>(s_CreateDlgTemplate(hb_param(1, Harbour::Item::OBJECT), hb_parni(2), hb_parni(3),
                                                      hb_parni(4), hb_parni(5), static_cast<ULONG>(hb_parnd(6)))));
}
...
HB_FUNC(HWG__CREATEPROPERTYSHEETPAGE)
{
  ...
  psp.pfnDlgProc = reinterpret_cast<DLGPROC>(s_PSPProc);
  psp.lParam = reinterpret_cast<LPARAM>(hb_itemNew(pObj));
  psp.pfnCallback = reinterpret_cast<LPFNPSPCALLBACK>(s_PSPProcRelease);
  psp.pcRefParent = 0;
  ...
  {
    pdlgtemplate = reinterpret_cast<LPDLGTEMPLATE>(hb_parnl(2));
...
HB_FUNC(HWG_CREATEDLGINDIRECT)
{
  ...
  {
    pdlgtemplate = reinterpret_cast<LPDLGTEMPLATE>(hb_parnl(8));
  }
  else
  {
    ULONG ulStyle = ((hb_pcount() > 6 && !HB_ISNIL(7))
                         ? static_cast<ULONG>(hb_parnd(7))
  ...
  CreateDialogIndirectParam(hModule, pdlgtemplate, hwg_par_HWND(1), reinterpret_cast<DLGPROC>(s_DlgProc),
                            reinterpret_cast<LPARAM>(pObject));
...
HB_FUNC(HWG_DLGBOXINDIRECT)
{
  auto pObject = hb_param(2, Harbour::Item::OBJECT);
  ULONG ulStyle =
      ((hb_pcount() > 6 && !HB_ISNIL(7)) ? static_cast<ULONG>(hb_parnd(7))
  ...
  DialogBoxIndirectParam(hModule, pdlgtemplate, hwg_par_HWND(1), reinterpret_cast<DLGPROC>(s_ModalDlgProc),
                         reinterpret_cast<LPARAM>(pObject));
  s_ReleaseDlgTemplate(pdlgtemplate);
}
...
static LRESULT CALLBACK s_PSPProcRelease(HWND hwnd, UINT uMsg, LPPROPSHEETPAGE ppsp)
{
  ...
  {
    hb_itemRelease(reinterpret_cast<PHB_ITEM>(ppsp->lParam));
  }
