...
{
  void *hStr;
  auto hBtn = CreateWindowEx(0, TEXT("BUTTON"), HB_PARSTR(8, &hStr, nullptr), WS_CHILD | WS_VISIBLE | hwg_par_DWORD(3),
                             hwg_par_int(4), hwg_par_int(5), hwg_par_int(6), hwg_par_int(7), hwg_par_HWND(1),
                             reinterpret_cast<HMENU>(hb_parni(2)), GetModuleHandle(nullptr), nullptr);
  hb_strfree(hStr);
  hb_retptr(hBtn);
}
...
{
  //   wpOrigButtonProc = static_cast<WNDPROC>(SetWindowLong(hwg_par_HWND(1), GWL_WNDPROC,
  //   static_cast<LONG>(ButtonSubclassProc)));
  wpOrigButtonProc = static_cast<LONG_PTR>(
      SetWindowLongPtr(hwg_par_HWND(1), GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(ButtonSubclassProc)));
}
