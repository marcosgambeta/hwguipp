...
HB_FUNC(HWG_NMCIOPEN)
{
  ...
  hb_retnl(
      mciSendCommand(0, MCI_OPEN, dwFlags, reinterpret_cast<DWORD_PTR>(static_cast<LPMCI_OPEN_PARMS>(&mciOpenParms))));
...
HB_FUNC(HWG_NMCIPLAY)
{
  ...
  hb_retnl(mciSendCommand(hb_parni(1), MCI_PLAY, dwFlags,
...
HB_FUNC(HWG_NMCIWINDOW)
{
  ...
  hb_retnl(mciSendCommand(hb_parni(1), MCI_WINDOW, MCI_ANIM_WINDOW_HWND | MCI_ANIM_WINDOW_DISABLE_STRETCH,
                          reinterpret_cast<DWORD_PTR>(static_cast<LPMCI_ANIM_WINDOW_PARMS>(&mciWindowParms))));
}
