...
#ifdef UNICODE
int ted_CalcItemWidth(TEDIT *pted, LPCTSTR szText, TEDATTR *pattr, int *iRealLen, int iWidth, short int bWrap,
                      short int bLastInFew)
#else
int ted_CalcItemWidth(TEDIT *pted, char *szText, TEDATTR *pattr, int *iRealLen, int iWidth, short int bWrap,
                      short int bLastInFew)
#endif
{
  int i, i1, iReal, xpos;
  ...
  // hwg_writelog( nullptr, "bLastInFew: %d iReal: %d iRealLen: %d iWidth: %d sz.cx
  // %d\r\n",bLastInFew,iReal,*iRealLen,iWidth,sz.cx );
  if (sz.cx > iWidth) {
    for (i = iReal - 1; i > 0; i--) {
      if (bWrap) {
        i1 = i;
        while (i > 0 && !strchr(szDelimiters, *(szText + i))) {
          i--;
        }
    ...
    xpos = sz.cx;
    *iRealLen = i;
    // hwg_writelog( nullptr, "> iRealLen: %d sz.cx %d\r\n",*iRealLen,sz.cx );
  } else {
    xpos = sz.cx;
    i1 = iReal;
    ...
    if (bWrap && iReal < *iRealLen) {
      i = i1;
      while (i && !strchr(szDelimiters, *(szText + i)) && !strchr(szDelimiters, *(szText + i - 1))) {
        i--;
      }
      if (i || bLastInFew) { // if( i )
        i1 = i;
    ...
    for (i = iReal + 1; i <= *iRealLen; i++) {
      if (bWrap) {
        while (i < *iRealLen && !strchr(szDelimiters, *(szText + i))) {
          i++;
        }
      }
      GetTextExtentPoint32(pted->hDCScr, szText, i, &sz);
  ...
  *iRealWidth = 0;
  for (i = 0, lasti = 0; i <= iLen; i++) {
    ...
    if (i == iLen || (pattr + i)->iFont != (pattr + lasti)->iFont) {
      iRealLen = i - lasti;
      // hwg_writelog( nullptr, "Lineout-1a %d %d %d \r\n", iRealLen, i, lasti );
      *iRealWidth += ted_CalcItemWidth(pted, szText + lasti, pattr + lasti, &iRealLen, iWidth - *iRealWidth, bWrap,
                                       (short int)lasti);
      iPrinted += iRealLen;
      if (iRealLen < i - lasti) {
        break;
  ...
  // hwg_writelog( nullptr, "Lineout-2 %d\r\n", iPrinted );
  return iPrinted;
}

#ifdef UNICODE
int ted_TextOut(TEDIT *pted, int xpos, int ypos, int iHeight, int iMaxAscent, LPCTSTR szText, TEDATTR *pattr, int iLen)
#else
...
#endif
{
  ...
  // hwg_writelog( nullptr, "len: %d text: %c%c%c \r\n", iLen, *( (char*)szText ), *( ((char*)szText) + 1), *(
  // ((char*)szText) + 2) );
...
#endif
{
  ...
  if (iPrinted) {
    for (i = 0, lasti = 0; i <= iPrinted; i++) {
      // if the color or font changes, then need to output
      if (i == iPrinted || (pattr + i)->fg != (pattr + lasti)->fg || (pattr + i)->bg != (pattr + lasti)->bg ||
        ...
        x1 += ted_TextOut(pted, x1, ypos, iHeight, iMaxAscent, szText + lasti, pattr + lasti, iRealLen);
        if (iRealLen != (i - lasti)) {
          break;
        }
        lasti = i;
...
TEDIT *ted_init(void)
{
  ...
  if (!bRegistered) {
    ...
    wndclass.hbrBackground = nullptr; //( HBRUSH ) 0;
...
{
  return CreateWindowEx((dwStyle & WS_BORDER) ? WS_EX_CLIENTEDGE : 0, TEXT("TEDIT"), TEXT(""), dwStyle, x, y, iWidth,
                        iHeight, hwndParent, (HMENU)(UINT_PTR)id, GetModuleHandle(0), 0);
}
...
{
  hb_retptr(
      ted_create(hwg_par_HWND(1), hb_parni(2), (DWORD)hb_parnl(3), hb_parni(4), hb_parni(5), hb_parni(6), hb_parni(7)));
}
...
HB_FUNC(HCED_SETFONT)
{
  ...
  ted_setfont((TEDIT *)hb_parptr(1), (HFONT)hb_parptr(2), iFont, (short int)((HB_ISNIL(4)) ? 0 : hb_parl(4)));
}
...
HB_FUNC(HCED_SETATTR)
{
  ...
  // hwg_writelog( nullptr, "%lu %lu\r\n", hb_parni(2),i );

  if (iPos + i >= pted->iAttrLen) {
    iLen = pted->iAttrLen;
    pted->iAttrLen = iPos + i + 128;
    // hwg_writelog( nullptr, "realloc %lu %lu\r\n", iLen,pted->iAttrLen );
    ...
    // hwg_writelog( nullptr, "realloc - Ok\r\n" );
  }
  ...
  // hwg_writelog( nullptr, "SetAttr - end\r\n" );
}
...
HB_FUNC(HCED_FILLRECT)
{
  ...
  if (!pted->hDCPrn) {
    SetRect(&rect, (HB_ISNIL(2)) ? 0 : hb_parni(2), hb_parni(3), (HB_ISNIL(4)) ? pted->iWidth : hb_parni(4),
            hb_parni(5));
    if (pted->bg != pted->bg_curr) {
      SetBkColor(pted->hDCScr, pted->bg);
      pted->bg_curr = pted->bg;
...
HB_FUNC(HCED_INITCARET)
{
  ...
  InvalidateRect(pted->handle, nullptr, 0);
}
...
HB_FUNC(HCED_EXACTCARETPOS)
{
  ...
  if (bSet) {
    ...
    if (iCaretHeight != pted->iCaretHeight) {
      ...
      CreateCaret(pted->handle, (HBITMAP) nullptr, 2, iCaretHeight);
      ShowCaret(pted->handle);
    }
    x1 -= iShiftL;
    SetCaretPos(x1, y1);
...
HB_FUNC(HCED_LINEOUT)
{
  ...
  if (iLen) {
    if (bCalc) {
      if (pted->iDocWidth) {
        ...
        // hwg_writelog( nullptr, "iWidth: %d iDocWidth: %d iRealWidth %d iPrinted %d \r\n", x2, pted->iDocWidth,
        // iRealWidth, iPrinted ); iPrinted = ted_CalcLineWidth( pted, szText, iCalculated, x2-x1, &iRealWidth, 0 );
      } else {
  ...
  */
  // hwg_writelog( nullptr, "ypos: %d iHeight: %d iLen %d iPrinted %d bCalc %d\r\n", ypos, iHeight, iLen, iPrinted,
  // bCalc );
  if (iRight) {
