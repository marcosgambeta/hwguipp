...
int ted_CalcItemWidth(PangoLayout *layout, char *szText, TEDFONT *font, int *iRealLen, int iWidth, HB_BOOL bWrap,
                      HB_BOOL bLastInFew)
{
  ...
  *iRealWidth = 0;
  for (i = 0, lasti = 0; i <= iLen; i++)
  {
    if (i == iLen || (pattr + i)->iFont != (pattr + lasti)->iFont)
    {
      ...
      iRealLen = iReqLen;
      *iRealWidth += ted_CalcItemWidth(pted->hDCScr->layout, ptr, pted->pFontsScr + (pattr + lasti)->iFont, &iRealLen,
...
{
  ted_setfont((TEDIT *)hb_parptr(1), (PHWGUI_FONT)hb_parptr(2), -1, 0);
}
...
HB_FUNC(HCED_SETFONT)
{
  ...
  ted_setfont((TEDIT *)hb_parptr(1), (PHWGUI_FONT)hb_parptr(2), iFont, (HB_ISNIL(4)) ? 0 : hb_parl(4));
}
...
HB_FUNC(HCED_SETPAINT)
{
  ...
  pted->hDCScr = (HB_ISNIL(2)) ? nullptr : (PHWGUI_HDC)hb_parptr(2);
  pted->hDCPrn = (HB_ISNIL(3)) ? nullptr : (PHWGUI_HDC)hb_parptr(3);
...
HB_FUNC(HCED_FILLRECT)
{
  ...
  if (!pted->hDCPrn)
  {
    ...
    hwg_setcolor(pted->hDCScr->cr, pted->bg);
    pted->bg_curr = pted->bg;
    cairo_rectangle(pted->hDCScr->cr, (gdouble)x1, (gdouble)y1, (gdouble)(x2 - x1 + 1),
  ...
  if (iLen > 0)
  {
    ...
    for (i = 0, lasti = 0; i <= iLen; i++)
    {
      if (i == iLen || (pattr + i)->iFont != (pattr + lasti)->iFont)
      {
        ...
        {
          j--;
          x1 += (pted->pFontsScr + (pattr + iPrinted)->iFont)->iSpace;
        }
        iPrinted += iRealLen;
        if (iRealLen < iReqLen)
        {
