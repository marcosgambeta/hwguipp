...
#define SKIPTABSPACES(sptr)                                                                                            \
  while ((unsigned long)(sptr - pStart) < ulDataLen &&                                                                 \
         (*sptr == ' ' || *sptr == '\t' || *sptr == '\r' || *sptr == '\n'))                                            \
  (sptr)++
#define SKIPCHARS(sptr)                                                                                                \
  while ((unsigned long)(sptr - pStart) < ulDataLen && *sptr != ' ' && *sptr != '\t' && *sptr != '=' &&                \
         *sptr != '>' && *sptr != '<' && *sptr != '\"' && *sptr != '\'' && *sptr != '\r' && *sptr != '\n')             \
  (sptr)++
...
static unsigned char *predefinedEntity1[] = {reinterpret_cast<unsigned char *>(const_cast<char *>("lt;")),
...
HB_FUNC(HBXML_PRELOAD)
{
  ...
  unsigned long ulLen =
      HB_ISNUM(2) ? static_cast<unsigned long>(hb_parnl(2)) : static_cast<unsigned long>(hb_parclen(1));
...
int hbxml_readComment(PHB_ITEM pParent, unsigned char **pBuffer)
{
  ...
  (*pBuffer) += 4;
  unsigned char *ptr = *pBuffer;
  while (static_cast<unsigned long>(*pBuffer - pStart) < ulDataLen &&
